...
import (
	...
	// "github.com/free5gc/uecmf/pkg/suci"
	...
	// 20241209 modified, cpaf is not on github, so we use the local one
	cpaf_factory "github.com/free5gc/cpaf/pkg/factory"
	cpaf_service "github.com/free5gc/cpaf/pkg/service"

	// 20250116 add, sdpaf is not on github, so we use the local one
	sdpaf_factory "github.com/free5gc/sdpaf/pkg/factory"
	sdpaf_service "github.com/free5gc/sdpaf/pkg/service"

	// 20250226 add, apcf is not on github, so we use the local one
	apcf_factory "github.com/free5gc/apcf/pkg/factory"
	apcf_service "github.com/free5gc/apcf/pkg/service"

	// 20250226 add, apcf is not on github, so we use the local one
	uecmf_factory "github.com/free5gc/uecmf/pkg/factory"
	uecmf_service "github.com/free5gc/uecmf/pkg/service"
...
type StartNFsConfig struct {
	...
	// 20241205 add for CPAF
	Cpaf bool `yaml:"cpaf,omitempty" default:"false"`
	...
	// 20250116 add for SDPAF
	Sdpaf bool `yaml:"sdpaf,omitempty" default:"false"`
	Apcf  bool `yaml:"apcf,omitempty" default:"false"`
	Uecmf bool `yaml:"Uecmf,omitempty" default:"false"`
...
func CreateNFs(cfg StartNFsConfig) []app.NFstruct {
	...
	// 20241205 add for CPAF
	if cfg.Cpaf {
		nfs = append(nfs, NewCpafStruct(NfCtx, cfg.TestId))
	}
	if cfg.Smf {
	...
	// 20250116 add for SDPAF
	if cfg.Sdpaf {
		nfs = append(nfs, NewSdpafStruct(NfCtx))
	}
	// 20250226 add for APCF
	if cfg.Apcf {
		nfs = append(nfs, NewApcfStruct(NfCtx))
	}
	// 20250226 add for APCF
	if cfg.Uecmf {
		nfs = append(nfs, NewUecmfStruct(NfCtx))
	}
	if cfg.Chf {
...
// 20241205 add for CPAF
func NewCpafStruct(ctx context.Context, testId TestId) app.NFstruct {
	if err := cpafConfig(testId); err != nil {
		fmt.Printf("CPAF Config failed: %v\n", err)
	}
	cpaf_ctx, cpaf_cancel := context.WithCancel(ctx)
	cpafApp, errApp := cpaf_service.NewApp(cpaf_ctx, cpaf_factory.AmfConfig, "")
	if errApp != nil {
		fmt.Printf("CPAF NewApp failed: %v\n", errApp)
	}
...
// 20250116 add for sdpaf
func NewSdpafStruct(ctx context.Context) app.NFstruct {
	if err := sdpafConfig(); err != nil {
		fmt.Printf("SDPAF Config failed: %v\n", err)
	}

	sdpaf_ctx, sdpaf_cancel := context.WithCancel(ctx)
	sdpafApp, errApp := sdpaf_service.NewApp(sdpaf_ctx, sdpaf_factory.AusfConfig, "")
	if errApp != nil {
		fmt.Printf("SDPAF NewApp failed: %v\n", errApp)
	}
...
func NewApcfStruct(ctx context.Context) app.NFstruct {
	if err := apcfConfig(); err != nil {
		fmt.Printf("APCF Config failed: %v\n", err)
	}
	apcf_ctx, apcf_cancel := context.WithCancel(ctx)
	apcfApp, errApp := apcf_service.NewApp(apcf_ctx, apcf_factory.PcfConfig, "")
	if errApp != nil {
		fmt.Printf("APCF NewApp failed: %v\n", errApp)
	}
...
func NewUecmfStruct(ctx context.Context) app.NFstruct {
	if err := uecmfConfig(); err != nil {
		fmt.Printf("UECMF Config failed: %v\n", err)
	}
	uecmf_ctx, uecmf_cancel := context.WithCancel(ctx)
	uecmfApp, errApp := uecmf_service.NewApp(uecmf_ctx, uecmf_factory.UdmConfig, "")
	if errApp != nil {
		fmt.Printf("UECMF NewApp failed: %v\n", errApp)
	}
...
// 20241205 add for CPAF
func cpafConfig(testID TestId) error {
	var ngapIpList []string
	...
	cpaf_factory.AmfConfig = &cpaf_factory.Config{
		Info: &cpaf_factory.Info{
			Version:     "1.0.0",
			Description: "CPAF initial test configuration",
		},
		Configuration: &cpaf_factory.Configuration{
			AmfName:    "CPAF",
			NgapIpList: ngapIpList,
			NgapPort:   38412,

			Sbi: &cpaf_factory.Sbi{
				Scheme:       "http",
				RegisterIPv4: "127.0.0.18",
				BindingIPv4:  "127.0.0.18",
				Port:         8000,
				Tls: &cpaf_factory.Tls{
					Pem: "cert/amf.pem",
					Key: "cert/amf.key",
				},
			},
			...
			PlmnSupportList: []cpaf_factory.PlmnSupportItem{{
				PlmnId: &models.PlmnId{
					Mcc: "208",
					Mnc: "93",
				},
			...
			Security: &cpaf_factory.Security{
				IntegrityOrder: []string{"NIA2"},
				CipheringOrder: []string{"NEA0", "NEA2"},
			},

			NetworkName: cpaf_factory.NetworkName{
				Full:  "free5GC",
				Short: "free",
			},

			NgapIE: &cpaf_factory.NgapIE{
				MobilityRestrictionList: &cpaf_factory.MobilityRestrictionList{
					Enable: true,
				},
				MaskedIMEISV: &cpaf_factory.MaskedIMEISV{
					Enable: true,
				},
				RedirectionVoiceFallback: &cpaf_factory.RedirectionVoiceFallback{
					Enable: false,
				},
			},

			NasIE: &cpaf_factory.NasIE{
				NetworkFeatureSupport5GS: &cpaf_factory.NetworkFeatureSupport5GS{
					Enable:  true,
					Length:  1,
					ImsVoPS: 0,
					Emc:     0,
	...
	if _, err := cpaf_factory.AmfConfig.Validate(); err != nil {
		return err
	}
...
// 20250116 add for sdpaf
func sdpafConfig() error {
	sdpaf_factory.AusfConfig = &sdpaf_factory.Config{
		Info: &sdpaf_factory.Info{
			Version:     "1.0.3",
			Description: "SDPAF initial test configuration",
		},
		Configuration: &sdpaf_factory.Configuration{
			Sbi: &sdpaf_factory.Sbi{
				Scheme:       "http",
				RegisterIPv4: "127.0.0.9",
				BindingIPv4:  "127.0.0.9",
				Port:         8000,
				Tls: &sdpaf_factory.Tls{
					Pem: "cert/ausf.pem",
					Key: "cert/ausf.key",
				},
			},
			...
			// ServiceList: []apcf_factory.Service{{
	...
	if _, err := sdpaf_factory.AusfConfig.Validate(); err != nil {
		return err
	}
...
// 20250226 add for apcf
func apcfConfig() error {
	apcf_factory.PcfConfig = &apcf_factory.Config{
		Info: &apcf_factory.Info{
			Version:     "1.0.2",
			Description: "APCF initial test configuration",
		},
		Configuration: &apcf_factory.Configuration{
			PcfName: "APCF",
			Sbi: &apcf_factory.Sbi{
				Scheme:       "http",
				RegisterIPv4: "127.0.0.7",
				BindingIPv4:  "127.0.0.7",
				Port:         8000,
				Tls: &apcf_factory.Tls{
					Pem: "cert/pcf.pem",
					Key: "cert/pcf.key",
				},
			},
			...
			Mongodb: &apcf_factory.Mongodb{
				Name: "free5gc",
				Url:  "mongodb://localhost:27017",
			},
			Locality: "area1",
		},
		Logger: &apcf_factory.Logger{
			Enable:       true,
			Level:        "info",
			ReportCaller: false,
		},
	...
	if _, err := apcf_factory.PcfConfig.Validate(); err != nil {
		return err
	}
...
func uecmfConfig() error {
	uecmf_factory.UdmConfig = &uecmf_factory.Config{
		Info: &uecmf_factory.Info{
			Version:     "1.0.3",
			Description: "UECMF initial test configuration",
		},
		Configuration: &uecmf_factory.Configuration{
			ServiceNameList: []string{
				"nudm-uecm",
				"nudm-ee",
				"nudm-pp",
			},
			Sbi: &uecmf_factory.Sbi{
				Scheme:       "http",
				RegisterIPv4: "127.0.0.3",
				BindingIPv4:  "127.0.0.3",
				Port:         8000,
				Tls: &uecmf_factory.Tls{
					Pem: "cert/udm.pem",
					Key: "cert/udm.key",
				},
			},
	...
	if _, err := uecmf_factory.UdmConfig.Validate(); err != nil {
		return err
	}
