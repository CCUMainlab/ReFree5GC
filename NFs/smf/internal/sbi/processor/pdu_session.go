...
func (p *Processor) HandlePDUSessionSMContextCreate(
	...
	smContext.Log.Infof("Starting PCF Selection for SUPI[%s] PDUSessionID[%d]", smContext.Supi, smContext.PDUSessionID)
	if err := p.Consumer().PCFSelection(smContext); err != nil {
		smContext.Log.Errorln("PCF selection error:", err)
	} else {
		smContext.Log.Infof("PCF Selection successful for SUPI[%s] PDUSessionID[%d]", smContext.Supi, smContext.PDUSessionID)
	}

	smContext.Log.Infof("Starting SM Policy Association Create for SUPI[%s] PDUSessionID[%d]", smContext.Supi, smContext.PDUSessionID)
