...
func (s *npcfService) SendSMPolicyAssociationCreate(smContext *smf_context.SMContext) (
	...
	if client == nil {
		logger.PduSessLog.Error("smContext not selected PCF")
	...
	ctx, _, err := smf_context.GetSelf().GetTokenCtx(models.ServiceName_NPCF_SMPOLICYCONTROL, models.NfType_AUSF)
	if err != nil {
		logger.PduSessLog.Error("GetTokenCtx")
		return "", nil, err
	}
	...
	if err != nil {
		logger.PduSessLog.Error("smPolicyDecisionFromPCF")
		return "", nil, err
	}
	...
	if smPolicyID = s.extractSMPolicyIDFromLocation(loc); len(smPolicyID) == 0 {
		logger.PduSessLog.Errorf("SMPolicy ID parse failed: %v", loc)
