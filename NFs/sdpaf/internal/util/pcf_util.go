...
// 20250326 add for SDPAF get ampolicy
func GetNpcfAMPolicyClient(uri string) *Npcf_AMPolicy.APIClient {
	configuration := Npcf_AMPolicy.NewConfiguration()
	configuration.SetBasePath(uri)
	client := Npcf_AMPolicy.NewAPIClient(configuration)
	return client
...
func GetResourceUri(name models.ServiceName, id string) string {
	...
	return fmt.Sprintf("%s/%s/%s", "http://127.0.0.9:8000/npcf-smpolicycontrol/v1", serviceUriMap[name], id)
