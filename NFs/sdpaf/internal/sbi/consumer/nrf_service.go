...
func (s *nnrfService) buildNfProfile(ausfContext *ausf_context.AUSFContext) (profile models.NfProfile, err error) {
	...
	if ausfContext.Locality != "" {
		profile.Locality = ausfContext.Locality
	}
	return
}
...
func (s *nnrfService) SendNFInstancesUDR(nrfUri string) string {
...
func (s *nnrfService) SendNFInstancesAPCF(nrfUri string) string {
	targetNfType := models.NfType_PCF
	requestNfType := models.NfType_AUSF
	localVarOptionals := Nnrf_NFDiscovery.SearchNFInstancesParamOpts{
		ServiceNames: optional.NewInterface([]models.ServiceName{models.ServiceName_NPCF_AM_POLICY_CONTROL}),
	}
	...
	for _, profile := range result.NfInstances {
		if uri := util.SearchNFServiceUri(profile, models.ServiceName_NPCF_AM_POLICY_CONTROL,
			models.NfServiceStatus_REGISTERED); uri != "" {
			return uri
		}
	}
...
func (s *nnrfService) SendNFInstancesUDR_forUDRUri(nrfUri, id string) string {
	targetNfType := models.NfType_UDR
	requestNfType := models.NfType_PCF
	localVarOptionals := Nnrf_NFDiscovery.SearchNFInstancesParamOpts{
