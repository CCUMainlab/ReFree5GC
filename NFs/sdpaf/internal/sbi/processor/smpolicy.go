...
import (
	...
	pcf_context "github.com/free5gc/sdpaf/internal/context"
...
func (p *Processor) HandleCreateSmPolicyRequest(
	...
	if val, exist := pcfSelf.UePool_PCF.Load(request.Supi); exist {
		ue = val.(*pcf_context.UeContext)
	}

	// 20250326 modified for SDPAF connect to APCF
	if ue == nil {
		apcfUri := p.Consumer().SendNFInstancesAPCF(pcfSelf.NrfUri)
		if apcfUri == "" {
			problemDetail := util.GetProblemDetail("Can't find APCF instance", util.USER_UNKNOWN)
			logger.SmPolicyLog.Warnf("Can't find APCF instance for UE[%s]", request.Supi)
			c.JSON(int(problemDetail.Status), problemDetail)
			return
		}

		apcfClient := util.GetNpcfAMPolicyClient(apcfUri)

		amPolicyReq := models.PolicyAssociationRequest{
			Supi:            request.Supi,
			AccessType:      request.AccessType,
			ServingPlmn:     request.ServingNetwork,
			NotificationUri: "http://localhost:29507/npcf-ampolicycontrol/v1/policies/notify",
		}

		ctx, pd, err := pcfSelf.GetTokenCtx(models.ServiceName_NPCF_AM_POLICY_CONTROL, models.NfType_PCF)
		if err != nil {
			c.JSON(int(pd.Status), pd)
			return
		}

		_, httpResp, err := apcfClient.DefaultApi.PoliciesPost(ctx, amPolicyReq)
		if err != nil || httpResp == nil || httpResp.StatusCode != http.StatusCreated {
			problemDetail := util.GetProblemDetail("Failed to get AM Policy from APCF", util.USER_UNKNOWN)
			logger.SmPolicyLog.Warnf("Failed to get AM Policy from APCF for UE[%s]", request.Supi)
			c.JSON(int(problemDetail.Status), problemDetail)
			return
		}
		...
		ue = &pcf_context.UeContext{
			...
			Gpsi:         request.Gpsi,
			Pei:          request.Pei,
			AMPolicyData: make(map[string]*pcf_context.UeAMPolicyData),
			SmPolicyData: make(map[string]*pcf_context.UeSmPolicyData),
		}

		assolId := fmt.Sprintf("%s-%d", ue.Supi, ue.PolAssociationIDGenerator)
		amPolicy := ue.NewUeAMPolicyData(assolId, amPolicyReq)
		amPolicy.AmPolicyData = &models.AmPolicyData{
			SubscCats: []string{"GOLD"},
		}
		ue.PolAssociationIDGenerator++

		pcfSelf.UePool_PCF.Store(request.Supi, ue)
		logger.SmPolicyLog.Infof("Created new UE context for SUPI[%s]", request.Supi)

		// ue.UdrUri = p.Consumer().SendNFInstancesUDR(p.Context().NrfUri, request.Supi)
	}
	...
	policyAuthorizationService, exists := p.Context().NfService[models.ServiceName_NPCF_POLICYAUTHORIZATION]
	if !exists {
		logger.SmPolicyLog.Warnf("Policy Authorization Service not found")
	} else {
...
func (p *Processor) getUdrUri(ue *pcf_context.UeContext) string {
	...
	udrUri := p.Consumer().SendNFInstancesUDR(p.Context().NrfUri)
	if udrUri != "" {
		ue.UdrUri = udrUri
		return udrUri
	}
