...
func (s *namfService) AmfStatusChangeSubscribe(amfUri string, guamiList []models.Guami) (
	...
	logger.ConsumerLog.Debugf("CPAF Subscribe to AMF status[%+v]", amfUri)
	amfContext := amf_context.GetSelf()

	// Set client and set url
	client := s.getComClient(amfUri)
	if client == nil {
		return nil, openapi.ReportError("amf not found")
	}

	subscriptionData := models.SubscriptionData{
		AmfStatusUri: fmt.Sprintf("%s/namf-callback/v1/amfstatus", amfContext.GetIPv4Uri()),
		GuamiList:    guamiList,
	}

	ctx, pd, err := amfContext.GetTokenCtx(models.ServiceName_NAMF_COMM, models.NfType_AMF)
	if err != nil {
		return pd, err
	}
	...
	if localErr == nil {
		...
		amfStatusSubsData := amf_context.AMFStatusSubscriptionData{
			AmfUri:       amfUri,
			AmfStatusUri: res.AmfStatusUri,
			GuamiList:    res.GuamiList,
		}
		amfContext.NewAmfStatusSubscription(subscriptionID, amfStatusSubsData)
	} else if httpResp != nil {
		if httpResp.Status != localErr.Error() {
			err = localErr
			return nil, err
