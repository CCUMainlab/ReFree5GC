...
import (
	...
	util_oauth "github.com/free5gc/cpaf/internal/util"
...
func newRouter(s *Server) *gin.Engine {
	...
	for _, serverName := range factory.AmfConfig.Configuration.ServiceNameList {
		...
		case models.ServiceName_NUDM_UEAU:
			...
			routerAuthorizationCheck := util_oauth.NewRouterAuthorizationCheck(models.ServiceName_NUDM_UEAU)
			udmUEAUGroup.Use(func(c *gin.Context) {
				routerAuthorizationCheck.Check(c, amf_context.GetSelf())
			})
			applyRoutes(udmUEAUGroup, udmUEAURoutes)

			genAuthDataPath := "/:supi/security-information/generate-auth-data"
			udmUEAUGroup.Any(genAuthDataPath, s.GenAuthDataHandlerFunc)
		// 20241126 add for pc5
		case models.ServiceName_NPCF_UE_POLICY_CONTROL:
			uePolicyRoutes := s.getUePolicyRoutes()
			uePolicyGroup := router.Group(factory.PcfUePolicyCtlResUriPrefix) // 使用傳入的 router 參數
			routerAuthorizationCheck := util_oauth.NewRouterAuthorizationCheck(models.ServiceName_NPCF_UE_POLICY_CONTROL)
