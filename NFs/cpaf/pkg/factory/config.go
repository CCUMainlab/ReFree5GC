...
func (c *Configuration) validate() (bool, error) {
	...
	// 	}
	// }
	// ServiceNameList validation (合併AMF和UDM的服務名稱)
	if c.ServiceNameList != nil {
		var errs govalidator.Errors
		validServices := map[string]bool{
			...
			"nudm-ueau": true,
			// 20241122 add for PC5
			"npcf-ue-policy-control": true,
		}

		for _, v := range c.ServiceNameList {
			if !validServices[v] {
				err := fmt.Errorf("invalid ServiceNameList: %s, value should be one of: "+
					"namf-comm, namf-evts, namf-mt, namf-loc, namf-oam, nudm-ueau, npcf-ue-policy-control", v)
				errs = append(errs, err)
			}
		}
		if len(errs) > 0 {
	...
	// PCF TimeFormat validation
	if result := govalidator.IsTime(c.TimeFormat, PcfTimeFormatLayout); !result {
		err := fmt.Errorf("Invalid TimeFormat: %s, should be in 2006-01-02 15:04:05 format.", c.TimeFormat)
		return false, err
	}

	// PCF ServiceList validation
	...
	// 20241108 SuciProfiles validation (copy from UDM)
